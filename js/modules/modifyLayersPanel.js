...
export function modifyLayersPanel() {
	...
	let layersList = document.getElementsByClassName('leaflet-control-layers-list')[0];
	let basemapsGroup = document.getElementsByClassName('leaflet-control-layers-base')[0];
	let overlaysGroup = document.getElementsByClassName('leaflet-control-layers-overlays')[0];
	let layersSep = document.getElementsByClassName('leaflet-control-layers-separator')[0];
	...
	basemapsGroup.insertAdjacentHTML('afterbegin', '<h6>Картографическая основа</h6>');
	...
	let layersPanelOpenBtn = document.getElementById('layers-panel-open-button');
	layersPanelOpenBtn.onclick = (e) => {
		layersContainer.classList.add('leaflet-control-layers-container-visible');
	};

	//Добавляем функционал кнопке закрытия
	let layersPanelCloseBtn = document.getElementById('layers-panel-close-button');
	layersPanelCloseBtn.onclick = (e) => {
		layersContainer.classList.remove('leaflet-control-layers-container-visible');
	};

	//Все спэны-слои
	let children = overlaysGroup.querySelectorAll('span:has(input.leaflet-control-layers-selector)');
	...
	for (let span of children) {
		...
		for (let layer in legendLayers) {
			...
			if (span.textContent.trim() == layer) {
				...
				span.getElementsByClassName('leaflet-control-layer-legend')[0].setAttribute('layer', layer);
				//Здесь же добавляем категории слоёв
				let groupName = cfg.layers.filter((lyr) => {
					return lyr.name == layer;
				})[0];
				if (groupName.groupHeader)
					span.parentElement.insertAdjacentHTML('beforebegin', `<h6>${groupName.group}</h6>`);
				break;
			}
		}
